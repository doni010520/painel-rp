    ...
    <Card className="flex items-center gap-4">
      <div
        className={`relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full text-white ${
          isIg ? "bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600" : "bg-green-500"
        }`}
      >
        {isIg ? <InstagramGlyph /> : <WhatsAppGlyph />}
        <span
          className={`absolute -bottom-1 rounded px-1 text-[8px] font-bold leading-tight ${
            isIg ? "bg-pink-600 text-white" : isMeta ? "bg-blue-600 text-white" : "bg-gray-700 text-white"
          }`}
        >
          {isIg ? "IG" : isMeta ? "Meta" : "WEB"}
        </span>
      </div>

      <div className="min-w-0 flex-1">
        <h3 className="truncate text-sm font-semibold text-ink">{channel.name}</h3>
        <p className="text-xs text-ink-soft">{isIg ? "Instagram Direct" : formatPhone(channel.phone)}</p>
      </div>
